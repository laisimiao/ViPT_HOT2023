...
import os
import vot
from lib.test.tracker.data_utils import PreprocessorHSI
...
class ViPTTrack(BaseTracker):
    def __init__(self, params):
        ...
        self.network.eval()
        self.preprocessor = PreprocessorHSI()
        self.state = None
    ...
    def track(self, image, info: dict = None, train_data_type=""):
        H, W, _ = image.shape
        self.frame_id += 1
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(
                template=self.z_tensor, search=x_tensor, ce_template_mask=self.box_mask_z, train_data_type=train_data_type)
