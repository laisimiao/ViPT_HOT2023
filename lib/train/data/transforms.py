...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        ...
        if dims[0] == 6:
            ...
            depth = tvisf.normalize(depth, self.mean, self.std, self.inplace)

            return torch.cat((rgb, depth), 0)
        if dims[0] == 19:
            rgb = image[:3, :, :]
            depth = image[3:, :, :]
            mean_hsi = self.mean * 7
            std_hsi = self.std * 7
            mean_hsi = mean_hsi[:16]
            std_hsi = std_hsi[:16]

            rgb = tvisf.normalize(rgb, self.mean, self.std, self.inplace)
            depth = tvisf.normalize(depth, mean_hsi, std_hsi, self.inplace)

            return torch.cat((rgb, depth), 0)
        if dims[0] == 28:
            rgb = image[:3, :, :]
            depth = image[3:, :, :]
            mean_hsi = self.mean * 9
            std_hsi = self.std * 9
            mean_hsi = mean_hsi[:25]
            std_hsi = std_hsi[:25]
