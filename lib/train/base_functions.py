...
from lib.train.dataset import HSI3D
...
def update_settings(settings, cfg):
    ...
    settings.train_data_type = getattr(cfg.TRAIN.PROMPT, "DATATYPE", "")
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val", "COCO17", "VID", "TRACKINGNET",
                        ...
                        "HSI_train", "HSI_val", "HSI_train_vis", "HSI_val_vis", 
                        "HSI_train_nir", "HSI_val_nir", "HSI_train_rednir", "HSI_val_rednir"]
        ...
        if name == "HSI_train_vis":
            datasets.append(HSI3D(settings.env.hsi_dir, split='train_vis', image_loader=image_loader))
        if name == "HSI_val_vis":
            datasets.append(HSI3D(settings.env.hsi_dir, split='val_vis', image_loader=image_loader))
        if name == "HSI_train_nir":
            datasets.append(HSI3D(settings.env.hsi_dir, split='train_nir', image_loader=image_loader))
        if name == "HSI_val_nir":
            datasets.append(HSI3D(settings.env.hsi_dir, split='val_nir', image_loader=image_loader))
        if name == "HSI_train_rednir":
            datasets.append(HSI3D(settings.env.hsi_dir, split='train_rednir', image_loader=image_loader))
        if name == "HSI_val_rednir":
            datasets.append(HSI3D(settings.env.hsi_dir, split='val_rednir', image_loader=image_loader))
        if name == "HSI_train":
            datasets.append(HSI3D(settings.env.hsi_dir, split='train', image_loader=image_loader))
        if name == "HSI_val":
            datasets.append(HSI3D(settings.env.hsi_dir, split='val', image_loader=image_loader))
    return datasets
...
def get_optimizer_scheduler(net, cfg):
    ...
    train_data_type = getattr(cfg.TRAIN.PROMPT, "DATATYPE", "")
    print(f"train_data_type: {train_data_type}")
    if 'vipt' in train_type:
        ...
        for n, p in net.named_parameters():
            if "prompt" in n and train_data_type in n:
